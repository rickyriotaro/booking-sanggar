...
@section('title', 'Detail Jasa Tari')
...
<div class="mb-6">
    ...
    <div class="flex justify-between items-center">
        <h1 class="text-2xl font-bold text-gray-900">Detail Jasa Tari</h1>
        <div class="flex gap-2">
            <a href="{{ route('admin.dance-services.edit', $danceService) }}" class="bg-yellow-600 hover:bg-yellow-700 text-white px-4 py-2 rounded-lg flex items-center gap-2">
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div class="lg:col-span-2 bg-white rounded-lg shadow overflow-hidden">
        ...
        @if($danceService->image_path)
            <img src="{{ asset('storage/' . $danceService->image_path) }}" alt="{{ $danceService->package_name }}" class="w-full h-64 object-cover">
        @else
        ...
        <div class="p-6">
            ...
            <div class="space-y-4">
                <div class="grid grid-cols-3 gap-4">
                    ...
                    <div class="col-span-2 font-medium text-gray-900">{{ $danceService->package_name }}</div>
                </div>

                <div class="grid grid-cols-3 gap-4">
                    <div class="text-sm text-gray-600">Tipe Tarian</div>
                    <div class="col-span-2 font-medium text-gray-900">{{ $danceService->dance_type }}</div>
                </div>

                <div class="grid grid-cols-3 gap-4">
                    <div class="text-sm text-gray-600">Jumlah Penari</div>
                    <div class="col-span-2">
                        <span class="bg-purple-100 text-purple-800 px-3 py-1 rounded font-medium">
                            {{ $danceService->number_of_dancers }} Penari
                        </span>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-3 gap-4">
                    ...
                    <div class="col-span-2 text-lg font-bold text-red-800">
                        Rp {{ number_format($danceService->price, 0, ',', '.') }}
                    </div>
                </div>
                ...
                <div class="grid grid-cols-3 gap-4">
                    ...
                    <div class="col-span-2 font-medium text-gray-900">
                        {{ $danceService->duration_minutes }} Menit
                    </div>
                </div>

                <div class="grid grid-cols-3 gap-4">
                    <div class="text-sm text-gray-600">Status Ketersediaan Tari</div>
                    <div class="col-span-2">
                        @if($danceService->is_available)
                ...
                @if($danceService->description)
                <div class="grid grid-cols-3 gap-4 pt-4 border-t">
                    <div class="text-sm text-gray-600">Deskripsi</div>
                    <div class="col-span-2 text-gray-900">{{ $danceService->description }}</div>
                </div>
                @endif
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow p-6">
            ...
            <div class="space-y-3">
                ...
                <div class="flex justify-between items-center">
                    ...
                    <span class="font-bold text-purple-600">{{ $danceService->views_count ?? 0 }} views</span>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow p-6">
            ...
            <div class="space-y-3">
                <div class="flex justify-between items-center">
                    ...
                    <span class="font-bold text-gray-900">{{ $danceService->orderDetails->filter(function($detail) {
                ...
                <div class="flex justify-between items-center">
                    ...
                    <span class="font-bold text-green-600">
                        Rp {{ number_format($danceService->orderDetails->filter(function($detail) {
                            return in_array($detail->order->status, ['paid', 'completed']);
                        })->sum(function($detail) {
        ...
        <div class="bg-white rounded-lg shadow p-6">
            ...
            <div class="space-y-3 text-sm">
                <div class="flex justify-between">
                    ...
                    <span class="text-gray-900">{{ $danceService->created_at->format('d M Y') }}</span>
                </div>
                <div class="flex justify-between">
                    <span class="text-gray-600">Terakhir Update</span>
                    <span class="text-gray-900">{{ $danceService->updated_at->format('d M Y') }}</span>
                </div>
            </div>
        </div>
    </div>
...
<div class="mt-6 bg-white rounded-lg shadow p-6">
    ...
    @if($danceService->orderDetails->count() > 0)
    <div class="overflow-x-auto">
        <table class="w-full">
            ...
            <tbody class="divide-y divide-gray-200">
                @foreach($danceService->orderDetails->take(10) as $detail)
    ...
    @if($danceService->orderDetails->count() > 10)
    <p class="text-sm text-gray-500 mt-3">Menampilkan 10 pesanan terakhir dari {{ $danceService->orderDetails->count() }} total pesanan</p>
    @endif
    @else
    <div class="text-center py-8 text-gray-500">
        ...
        <p class="mt-2">Belum ada pesanan untuk jasa tari ini</p>
    </div>
    @endif
</div>
@endsection
